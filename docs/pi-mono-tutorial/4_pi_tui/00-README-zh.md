...
> 原文：[packages/tui/README.md](../../packages/tui/README.md)

@mariozechner/pi-tui

极简终端 UI 框架，具有差分渲染和同步输出功能，用于无闪烁的交互式 CLI 应用程序。

## 特性

- **差分渲染**：三策略渲染系统，只更新变化的内容
- **同步输出**：使用 CSI 2026 进行原子屏幕更新（无闪烁）
- **括号粘贴模式**：正确处理大段粘贴，对超过 10 行的粘贴进行标记
- **基于组件**：简单的 Component 接口，带有 render() 方法
- **主题支持**：组件接受主题接口以进行可定制的样式
...
覆盖层在现有内容之上渲染组件而不替换它。用于对话框、菜单和模态 UI。
...
const handle = tui.showOverlay(component, {
  ...
  minWidth: 40,           // 最小宽度下限
  maxHeight: 20,          // 最大高度（行）
  maxHeight: "50%",       // 最大高度作为终端百分比

  // 基于锚点的定位（默认：'center'）
  anchor: 'bottom-right', // 相对于锚点定位
  offsetX: 2,             // 锚点水平偏移
  offsetY: -1,            // 锚点垂直偏移
...
handle.focus();             // 聚焦并带到视觉前面
handle.unfocus();           // 将焦点释放给前一个目标
handle.isFocused();         // 检查覆盖层是否有焦点
...
| `handleInput?(data)` | 当组件有焦点并接收键盘输入时调用。`data` 字符串包含原始终端输入（可能包含 ANSI 转义序列）。|
| `invalidate?()` | 调用以清除任何缓存的渲染状态。组件应在下一次 `render()` 调用时从头开始重新渲染。|

TUI 在每行渲染末尾追加完整的 SGR 重置和 OSC 8 重置。样式不会跨行继承。如果你发出带样式的多行文本，每行重新应用样式或使用 `wrapTextWithAnsi()` 以便为每行保留样式。
...
这使得 IME 候选窗口出现在 CJK 输入法的正确位置。`Editor` 和 `Input` 内置组件已实现此接口。

**带有嵌入输入的容器组件：** 当容器组件（对话框、选择器等）包含 `Input` 或 `Editor` 子组件时，容器必须实现 `Focusable` 并将焦点状态传播给子组件：
...
没有此传播，使用 IME（中文、日文、韩文等）输入时，候选窗口将显示在错误位置。
...
带自动补全、文件补全、粘贴处理和垂直滚动的多行文本编辑器，当内容超过终端高度时。
...
const editor = new Editor(tui, theme, options?);  // tui 是高度感知滚动所必需的
...
- `Ctrl+A` / `Ctrl+E` - 行首/行尾
- `Ctrl+]` - 向前跳转到字符（等待下一次按键，然后移动光标到第一次出现的位置）
- `Ctrl+Alt+]` - 向后跳转到字符
- 方向键、Backspace、Delete 按预期工作

### Markdown

使用语法高亮和主题支持渲染 Markdown。
...
扩展 Loader，带有 Escape 键处理和用于取消异步操作的 AbortSignal。
...
interface SettingItem {
  ...
  description?: string;
  currentValue: string;
  values?: string[];  // 如果提供，Enter/Space 在这些之间循环
...
const settings = new SettingsList(
  ...
  (id, newValue) => console.log(`${id} 更改为 ${newValue}`),
  () => console.log("已取消")
);
...
为支持 Kitty 图形协议（Kitty、Ghostty、WezTerm）或 iTerm2 内联图片的终端内联渲染图片。在不支持的终端上回退到文本占位符。
...
支持格式：PNG、JPEG、GIF、WebP。尺寸自动从图片头解析。
...
使用 `matchesKey()` 和 `Key` 辅助函数检测键盘输入（支持 Kitty 键盘协议）：
...
1. **首次渲染**：输出所有行而不清除回滚
2. **宽度更改或视口上方更改**：清除屏幕并完全重新渲染
3. **正常更新**：将光标移动到第一行更改处，清除到末尾，渲染更改的行

所有更新都包裹在**同步输出**（`\x1b[?2026h` ... `\x1b[?2026l`）中，以实现原子、无闪烁渲染。

## Terminal 接口

TUI 与任何实现 `Terminal` 接口的对象一起工作：
...
// 将字符串截断到宽度（保留 ANSI 代码，添加省略号）
const truncated = truncateToWidth("你好世界", 8); // "你好..."

// 不带省略号截断
const truncatedNoEllipsis = truncateToWidth("你好世界", 8, ""); // "你好世"

// 将文本换行到宽度（跨行保留 ANSI 代码）
const lines = wrapTextWithAnsi("这是一行很长的文本，需要换行", 20);
// ["这是一行很长的文本", "需要换行"]
```

## 创建自定义组件

创建自定义组件时，**`render()` 返回的每行不得超过 `width` 参数**。如果任何行比终端宽，TUI 会报错。
...
`visibleWidth()` 和 `truncateToWidth()` 都正确处理 ANSI 转义代码：

- `visibleWidth()` 计算宽度时忽略 ANSI 代码
...
const truncated = truncateToWidth(styled, 8); // 红色 "你好" + " 世..." 带正确重置
```

### 缓存

为了性能，组件应缓存其渲染输出，仅在必要时重新渲染：
...
查看 `test/chat-simple.ts` 获取完整的聊天界面示例，包括：
