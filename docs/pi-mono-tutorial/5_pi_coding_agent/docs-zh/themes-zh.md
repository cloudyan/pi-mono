...
主题是定义 TUI（Terminal User Interface，终端用户界面）颜色的 JSON 文件。
...
**热重载：** 当你编辑当前活动的自定义主题文件时，pi 会自动重新加载以提供即时视觉反馈。
...
- `name` 是必需的，且必须唯一。
- `vars` 是可选的。在此定义可重用的颜色，然后在 `colors` 中引用它们。
...
**深色终端：** 使用明亮、高饱和度的颜色，对比度更高。

**浅色终端：** 使用较深、柔和的颜色，对比度较低。

**配色和谐：** 从基础调色板开始（Nord、Gruvbox、Tokyo Night），在 `vars` 中定义，并保持一致的引用。

**测试：** 用不同的消息类型、工具状态、Markdown 内容和长文本换行来检查你的主题。

**VS Code：** 将 `terminal.integrated.minimumContrastRatio` 设置为 `1` 以获得准确的颜色显示。
