...
技能是 agent 按需加载的独立能力包。一个技能为特定任务提供专门的工作流程、设置说明、辅助脚本和参考文档。

Pi 实现了 [Agent Skills 标准](https://agentskills.io/specification)，会警告违规情况但保持宽松处理。
...
- 项目：
  - `.pi/skills/`
  - `.agents/skills/` 位于 `cwd` 及其祖先目录（向上至 git 仓库根目录，或不在仓库中时至文件系统根目录）
...
技能是一个包含 `SKILL.md` 文件的目录。其他内容自由组织。
...
| `description` | 是 | 最多 1024 个字符。技能的功能及使用场景。 |
| `license` | 否 | 许可证名称或捆绑文件的引用。 |
| `compatibility` | 否 | 最多 500 个字符。环境要求。 |
| `metadata` | 否 | 任意键值映射。 |
| `allowed-tools` | 否 | 空格分隔的预批准工具列表（实验性）。 |
| `disable-model-invocation` | 否 | 为 `true` 时，技能在系统提示中隐藏。用户必须使用 `/skill:name`。 |
...
Pi 根据 Agent Skills 标准验证技能。大多数问题产生警告但仍会加载技能：
...
名称冲突（来自不同位置的相同名称）会警告并保留第一个找到的技能。
