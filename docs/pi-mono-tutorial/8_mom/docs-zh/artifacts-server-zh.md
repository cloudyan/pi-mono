...
通过 Cloudflare Tunnel 公开分享 HTML 文件、可视化和交互式演示，支持实时重载。

## 是什么？

Artifacts 服务器让 Mom 可以创建 HTML/JS/CSS 文件，你可以立即在浏览器中查看，并提供基于 WebSocket 的实时重载功能用于开发。非常适合仪表板、可视化、原型和交互式演示。
...
将以下内容保存为 `/workspace/artifacts/server.js`：
...
将以下内容保存为 `/workspace/artifacts/start-server.sh`：
...
2. 创建带有公开 URL 的 Cloudflare Tunnel
3. 打印 URL（例如 `https://random-words-123.trycloudflare.com`）
4. 将 URL 保存到 `/tmp/artifacts-url.txt`

**注意：** 每次重启时 URL 都会变化（免费 Cloudflare Tunnel 的限制）。

### 创建 Artifacts

**文件夹组织：**
- 每个 artifact 创建一个子文件夹：`$(date +%Y-%m-%d)-description/`
- 将主文件命名为 `index.html` 以获得简洁的 URL
- 图片、CSS、JS、数据放在同一文件夹
- CDN 资源（Tailwind、Three.js 等）可以正常使用
...
**访问：**
- **重要：** 实时重载功能需要使用完整的 `index.html` 路径
...
使用 `?ws=true` 查看时：
1. 你会在左下角看到绿色框："Live reload connected!"
2. 编辑 artifact 文件夹中的任何文件
3. 页面在 1 秒内自动重载
4. 非常适合迭代设计

**分享时移除 `?ws=true`** — 这样查看者没有 WebSocket 开销。

## 工作原理

**架构：**
- Node.js 服务器（Express）从 `/workspace/artifacts/files/` 提供静态文件
- Chokidar 文件监视器监控变更（包括新目录）
- WebSocket 向连接的客户端广播重载消息
- Cloudflare Tunnel 将 localhost 暴露到互联网，提供公开 HTTPS URL
- 客户端脚本在检测到文件变更时自动重载浏览器

**安全性：**
- 路径遍历保护防止访问 `files/` 目录之外的文件
- 只提供 `/workspace/artifacts/files/` 中的文件
- 缓存清除标头防止过期内容
...
**502 Bad Gateway：**
- Node 服务器崩溃了。检查日志：`cat /tmp/server.log`
...
**你：** "创建一个带有 Tailwind UI 的 Three.js 旋转立方体演示"

**Mom 创建：**
```
/workspace/artifacts/files/2025-12-14-threejs-cube/
├── index.html (来自 CDN 的 Three.js，来自 CDN 的 Tailwind)
└── screenshot.png
```
...
**结果：** 你的浏览器自动重载，显示带网格的紫色立方体（1 秒内）

## 技术说明

**为什么不用 Node.js fs.watch？**
- 带有 `recursive: true` 的 `fs.watch` 只在 macOS/Windows 上有效
- 在 Linux（Docker）上，它不支持递归监视
- Chokidar 是最可靠的跨平台解决方案
- 我们在检测到新目录时显式添加，以确保监视

**WebSocket vs Server-Sent Events：**
- WebSocket 通过 Cloudflare Tunnel 可靠工作
- 所有连接的客户端在任何文件变更时都会重载（简单方案）
- 生产环境中，你可以按当前页面路径过滤

**Cloudflare Tunnel 免费版：**
- 每次重启时随机子域名会变化
- 没有付费账户就没有持久 URL
- WebSocket 支持可靠，尽管是免费版