...
[Qwen3-Coder](https://github.com/QwenLM/Qwen3-Coder) 是阿里云 Qwen 团队创建的高级大语言模型(Large Language Model, LLM)。vLLM 已经支持 Qwen3-Coder, 而 `tool-call`(工具调用)功能将在 vLLM v0.10.0 及更高版本中可用。你可以使用以下方法安装支持 `tool-call` 的 vLLM:
...
### 在 8xH200 (或 H20) GPU(141GB × 8)上运行
...
### 评估
我们使用 vLLM 启动了 `Qwen3-Coder-480B-A35B-Instruct-FP8` 并使用 [EvalPlus](https://github.com/evalplus/evalplus) 评估其性能。结果如下所示:
...
### 基准测试
我们使用以下脚本来对 `Qwen/Qwen3-Coder-480B-A35B-Instruct-FP8` 进行基准测试
...
### FP8 模型
- **上下文长度限制**: 单个 H20 节点无法提供原始上下文长度(262144)。你可以减少 `max-model-len` 或增加 `gpu-memory-utilization` 以在内存限制内工作。
- **DeepGEMM 使用**: 要使用 [DeepGEMM](https://github.com/deepseek-ai/DeepGEMM), 设置 `VLLM_USE_DEEP_GEMM=1`。按照[安装说明](https://github.com/vllm-project/vllm/blob/main/benchmarks/kernels/deepgemm/README.md#setup)进行安装。
- **张量并行问题**: 当使用 `tensor-parallel-size 8` 时, 预计会出现以下失败。使用 `--data-parallel-size` 切换到数据并行模式。
- **其他资源**: 有关更多并行组的信息, 请参阅[数据并行部署文档](https://docs.vllm.ai/en/latest/serving/data_parallel_deployment.html)。
...
### 工具调用
- **启用工具调用**: 添加 `--tool-call-parser qwen3_coder` 以启用工具调用解析功能, 请参考: [tool_calling](https://docs.vllm.ai/en/latest/features/tool_calling.html)
