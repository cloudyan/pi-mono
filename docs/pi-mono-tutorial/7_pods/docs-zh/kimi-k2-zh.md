...
> [!Note]
> 本指南仅提供一些 Kimi-K2 的部署命令示例,这些可能不是最优配置。由于推理引擎仍在频繁更新,如果你希望获得更好的推理性能,请继续遵循其首页的指导。

## vLLM 部署
需要 vLLM 版本 v0.10.0rc1 或更高版本。

在主流 H200 或 H20 平台上,使用 128k 序列长度的 Kimi-K2 FP8 权重的最小部署单元是一个包含 16 个 GPU 的集群,可以使用 Tensor Parallel(Tensor Parallel,张量并行,TP)或"data parallel + expert parallel"(数据并行+专家并行,DP+EP)。
下面提供了此环境的运行参数。你可以扩展到更多节点并增加专家并行度,以扩大推理批次大小和整体吞吐量。
...
**关键参数说明:**
- `--tensor-parallel-size 16`: 如果使用超过 16 个 GPU,请与 pipeline-parallelism(流水线并行)结合使用。
- `--enable-auto-tool-choice`: 启用工具使用时需要此参数。
- `--tool-call-parser kimi_k2`: 启用工具使用时需要此参数。

### 数据并行 + 专家并行

你可以根据需要安装 DeepEP 和 DeepGEMM 等库。然后运行以下命令(H200 上的示例):
...
类似地,我们可以在 SGLang 中使用 TP 或 DP+EP 进行部署,以下是一些示例。
...
以下是在 SGLang 中使用 DP+EP 进行大规模 Prefill-Decode Disaggregation(4P12D H200)的示例:
...
## TensorRT-LLM 部署
### 先决条件
请参考[此指南](https://nvidia.github.io/TensorRT-LLM/installation/build-from-source-linux.html)从源代码构建 TensorRT-LLM v1.0.0-rc2 并启动 TRT-LLM docker 容器。
...
```
### 多节点服务
TensorRT-LLM 支持多节点推理。你可以使用 mpirun 启动多节点任务的 Kimi-K2。我们将以两个节点为例进行说明。

#### mpirun
mpirun 要求每个节点都能通过无密码 SSH 访问其他节点。我们需要在 docker 容器内设置环境。使用主机网络运行容器,并将当前目录和模型目录挂载到容器中。
...
完成准备工作后,你可以使用 mpirun 在两个节点上运行 trtllm-serve:
...
Kimi-K2 重用了 `DeepSeekV3CausalLM` 架构,并将其权重转换为适当形状以减少重新开发的工作量。为了让推理引擎将其与 DeepSeek-V3 区分开来并应用最佳优化,我们在 `config.json` 中设置了 `"model_type": "kimi_k2"`。

如果你使用的框架不在推荐列表中,你仍然可以通过在 `config.json` 中将 `model_type` 手动更改为 "deepseek_v3" 作为临时解决方案来运行模型。如果框架中没有可用的工具调用解析器,你可能需要手动解析工具调用。